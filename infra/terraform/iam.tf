...
resource "aws_iam_role_policy" "lambda" {
  ...
  policy = jsonencode({Version="2012-10-17",Statement=[
    ...
    {Effect="Allow",Action=["s3:GetObject","s3:PutObject"],Resource="${aws_s3_bucket.inference.arn}/*"},
    {Effect="Allow",Action=["sagemaker:InvokeEndpointAsync"],Resource="arn:aws:sagemaker:${var.aws_region}:${data.aws_caller_identity.current.account_id}:endpoint/${var.sagemaker_endpoint_name}"}
  ]})
}
...
resource "aws_iam_role_policy" "sagemaker" {
  ...
  policy = jsonencode({Version="2012-10-17",Statement=[
    {Effect="Allow",Action=["s3:GetObject","s3:PutObject","s3:ListBucket"],Resource=[aws_s3_bucket.dataset.arn,"${aws_s3_bucket.dataset.arn}/*",aws_s3_bucket.inference.arn,"${aws_s3_bucket.inference.arn}/*"]},
    ...
    {Effect="Allow",Action=["sns:Publish"],Resource=[aws_sns_topic.inference_success.arn,aws_sns_topic.inference_failure.arn]}
  ]})
}
