...
resource "aws_s3_bucket" "dataset" {
  bucket_prefix = "${var.project_name}-dataset-"
}
...
resource "aws_s3_bucket" "inference" {
  bucket_prefix = "${var.project_name}-inference-"
}
...
resource "aws_sns_topic" "inference_success" {
  name = "${var.project_name}-inference-success"
}

resource "aws_sns_topic" "inference_failure" {
  name = "${var.project_name}-inference-failure"
}
